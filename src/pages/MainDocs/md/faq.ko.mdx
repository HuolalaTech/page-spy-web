...
- 브라우저 SDK의 호환성 목표는 [`["chrome > 75","safari > 12", "> 0.1%", "not dead","not op_mini all"]`](https://github.com/HuolalaTech/page-spy/blob/main/packages/page-spy-browser/package.json#L66-L72)로 설정되어 있습니다. 다른 SDK는 각각의 저장소를 참조하세요.
- 디버깅 클라이언트는 주로 개발자용이며, 브라우저의 새로운 기능 사용에 대해 개방적인 태도를 유지합니다. 최신 브라우저 버전 사용을 권장합니다. 호환성 목표는 [`["last 2 chrome version", "last 2 firefox version", "last 2 safari version"]`](https://github.com/HuolalaTech/page-spy-web/blob/main/package.json#L92-L96)로 설정되어 있습니다.
...
PageSpy는 CodeSandbox 플랫폼에서 주요 프레임워크의 통합 가이드를 공개했습니다. 온라인으로 체험해 보세요:
...
### pagespy.jikejishu.com은 공식 제공 도메인인가요?#test-domain

[https://pagespy.jikejishu.com](https://pagespy.jikejishu.com)은 PageSpy를 온라인으로 체험하고 학습할 수 있도록 설정한 임시 서비스입니다. **24/7 가용성, 데이터 보안을 보장하지 않으며 발생하는 손실에 대해 책임지지 않습니다**. 체험 후 프라이빗 서버나 인트라넷에 배포할 것을 강력히 권장합니다.

### 로컬 6752 포트에는 접근할 수 있지만 서버에 배포하면 안 되나요?#server-port

서버의 방화벽 또는 보안 그룹 규칙에서 6752 포트가 개방되어 있는지 확인하세요.

### 디버그 버튼에 "현재 연결에 클라이언트가 존재하지 않습니다"는 무슨 의미인가요?#debug-disabled

이는 일반적으로 SDK가 정상적으로 룸을 생성했지만 WebSocket을 통해 룸에 참여하지 못한 경우 발생합니다. 다음 문제 해결 단계를 따르세요:

- SDK가 실행 중인 클라이언트에서 콘솔을 열고 오류를 확인하세요
- 콘솔에 "WebSocket connect failed" 관련 메시지가 표시되면 서버 구성이 올바른지 확인하세요
...
설정을 조정한 후 Nginx를 재시작하면 서브 경로로 접근할 수 있습니다. 주의할 점은 인스턴스화 시 `api`와 `clientOrigin` 매개변수를 수동으로 전달하여 SDK에 배포 주소를 알려야 합니다. 예:
...
### 디버그 클라이언트를 인증으로 보호하여 승인된 개발자만 접근할 수 있도록 하는 방법은?#security

[2.3.0](./changelog#v2-3-0) 버전부터 PageSpy는 디버깅 패널을 보호하기 위한 비밀번호 설정을 지원합니다. 비밀번호를 설정하면 개발자는 올바른 비밀번호를 입력해야만 디버깅 패널에 접근할 수 있습니다.

구성 가능한 변수:

- `AUTH_PASSWORD`: 비밀번호 설정
- `JWT_SECRET`: Token 비밀 키 설정
- `JWT_EXPIRATION_HOURS`: Token 만료 시간(시간) 설정, 기본값은 24시간
...
### 프로젝트에 수동으로 통합하고 싶지 않은 경우, 비즈니스 코드를 침해하지 않고 가능한가요?#extension

PageSpy는 다음 기능을 갖춘 브라우저 확장 프로그램을 제공합니다:
...
### 비즈니스 프로젝트가 HTTPS에 배포되어 있고 PageSpy가 HTTP에 배포되어 있을 때 콘솔 오류를 어떻게 해결하나요?#http-error

브라우저는 HTTPS 사이트에서 HTTP 리소스 로딩을 차단합니다. 이는 HTTPS는 암호화와 보안을 제공하지만 HTTP는 평문 전송으로 보안 위험이 있기 때문입니다.

PageSpy를 HTTPS 서비스로 업그레이드하면 이 문제를 완벽하게 해결할 수 있습니다.

### 특정 사용자만 디버그하려면 어떻게 하나요?#prod-debug

가장 간단한 방법은 사용자가 [PageSpy 브라우저 확장 프로그램](https://github.com/HuolalaTech/page-spy-extension)을 사용하는 것입니다. 이는 협조적인 PC 클라이언트에 적합하지만, 이 전제 조건은 상당히 까다롭습니다.

그렇다면 프로덕션 환경의 H5 프로젝트에서 PageSpy를 사용하려면 어떻게 해야 할까요? 모든 사용자에게 활성화하는 것은 분명히 비현실적입니다.

PageSpy의 활성화 프로세스는 두 단계만 포함합니다:
...
### Page 패널 스타일이 올바르지 않나요?#page-style

클라이언트와 디버깅 엔드의 렌더링 환경 차이(예: 클라이언트의 브라우저 버전이 Chrome 75이고 디버깅 엔드가 Chrome 120인 경우) 또는 디버깅 엔드가 클라이언트가 참조하는 리소스에 접근할 때 네트워크 제한으로 인해 스타일 차이가 발생할 수 있습니다.

따라서 스타일은 참고용으로만 제공됩니다.

### Page 패널이 클라이언트 콘텐츠를 100% 복원할 수 없나요?#page-reset

SDK는 페이지를 "스크린샷"하여 디버깅 엔드로 전송할 수 있지만 다음과 같은 이유로 구현하지 않았습니다:

- 이미지는 텍스트보다 데이터 크기가 커서 네트워크 전송 오버헤드가 증가함
- SDK의 크기와 복잡도가 증가함
- 스타일 오류의 경우 원격 협업 시 테스터가 개발자에게 정확한 피드백을 제공할 수 있음

이러한 이유로 Page 패널 스타일은 참고용으로만 제공됩니다.
...
- 룸 생성 후 SDK나 디버깅 엔드가 입장하지 않으면 1분 후 파괴됩니다(실제 사용에서는 이 시나리오가 존재하지 않습니다)
- SDK와 디버깅 엔드가 모두 연결이 끊어지면 1분 후 파괴됩니다
...
- 업로드된 파일 로그는 기본적으로 최신 10GB 및 30일 동안 저장되며, 구성을 통해 사용자 정의 가능
- 업로드 로그는 실행 디렉토리의 `log` 디렉토리에 저장됩니다. Docker로 실행하는 경우 컨테이너가 삭제되면 로그도 함께 삭제됩니다. `-v ./log:/app/log -v ./data:/app/data` 디렉토리 매핑을 사용하여 영구 저장할 수 있습니다

### 대규모 로그를 어떻게 처리할 수 있나요?#large-log

PageSpy는 사용자 작업、인터페이스 변경、콘솔、네트워크 요청 등을 포함하여 다양한 로그를 수집합니다. 대규모 로그에 대해서는 다양한 차원의 솔루션을 제공합니다:

- 일부 플러그인 비활성화, 클릭하여 보기: [config.disabledPlugins](./pagespy#config-disabledPlugins)
- 유용하지 않은 데이터 필터링, 클릭하여 보기: [config.dataProcessor](./pagespy#config-dataProcessor)
- 주기적으로 로그 재생, 클릭하여 보기: [수동 로그 작업](./data-harbor#manual)