...
- 浏览器 SDK 的兼容性目标为 [`["chrome > 75","safari > 12", "> 0.1%", "not dead","not op_mini all"]`](https://github.com/HuolalaTech/page-spy/blob/main/packages/page-spy-browser/package.json#L66-L72)，其他 SDK 可前往各自的仓库查看。
- 调试端主要供开发者使用，对于浏览器的新特性使用我们保持开放的态度。建议使用最新版本的浏览器，兼容性目标为 [`["last 2 chrome version", "last 2 firefox version", "last 2 safari version"]`](https://github.com/HuolalaTech/page-spy-web/blob/main/package.json#L92-L96)。
...
### 实例化可以传哪些参数，它们的作用是什么？#init-params
...
### pagespy.jikejishu.com 是官方提供的域名吗？#test-domain
...
### 本地 6752 端口可以访问，部署到服务器上就不行了？#server-port
...
在调整配置后，重启 Nginx 即可通过子路径访问。需要注意的是，实例化时需要手动传参 `api` 和 `clientOrigin` 告诉 SDK 部署地址，例如：
...
### 调试端如何加安全认证的保护，开发者通过认证才可以访问？#security
...
### 不想在项目里手动集成，是否可以不侵入业务代码？#extension
...
### 业务项目部署在 HTTPS，PageSpy 部署在 HTTP，控制台报错如何解决？#http-error
...
最简单的方案是让用户使用 [PageSpy 的浏览器插件](https://github.com/HuolalaTech/page-spy-extension)，适用于配合度高的 PC 端客户，但这个前提条件较为苛刻。

如果 H5 项目上线到生产环境想使用 PageSpy，对所有用户都开启显然不现实。

PageSpy 的生效过程总共只有两个步骤：
...
- 上传的文件日志默认最多保存最新的 10GB，以及 30 天，可以通过修改配置来自定义
- 上传日志保存在运行目录的 `log` 目录下，Docker 运行时，如果容器被销毁日志也会丢失，可以用目录映射 `-v ./log:/app/log -v ./data:/app/data` 来持久化

### 日志量很大，该如何处理？#large-log

PageSpy 收集的日志包括不限于：用户操作 / 页面变化 / console / 网络请求等等，对于大日志我们提供了不同维度的解决方案：

- 禁用部分插件，点击查看：[config.disabledPlugins](./pagespy#config-disabledPlugins)
- 过滤掉无用的数据，点击查看：[config.dataProcessor](./pagespy#config-dataProcessor)
- 定期重新制作日志，点击查看：[手动操作日志](./data-harbor#manual)