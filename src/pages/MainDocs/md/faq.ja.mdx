...
### 大きいログをどのように処理しますか？#large-log

PageSpyは、ユーザー操作/インターフェース変更/コンソール/ネットワークリクエストなどを含む、さまざまなログを収集します。大きいログについては、異なる次元のソリューションを提供しています：ネットワークリクエストなどを含む、さまざまなログを収集します。大きなログについては、異なる次元のソリューションを提供しています。

- 一部のプラグインを無効にする。クリックして表示：[config.disabledPlugins](./pagespy#config-disabledPlugins)
- 不要なデータをフィルタリングする。クリックして表示：[config.dataProcessor](./pagespy#config-dataProcessor)
- 定期的にログを再生成する。クリックして表示：[手動操作ログ](./data-harbor#manual)