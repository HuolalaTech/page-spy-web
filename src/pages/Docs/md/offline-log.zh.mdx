...
> 如果仅需要 PageSpy 的离线日志回放功能，我们推荐 [回放实验室](#replay-lab) 的使用方式，集成更简单。
...
[回放实验室](/replay-lab) 的 SDK 打包了 PageSpy / DataHarborPlugin / RRWebPlugin，内置 PageSpy 在离线状态下的最佳实践配置、同时支持自定义主题，使用非常简单。

与框架无关，你可以任选一种方式在项目中引入。

import { ImportGuide } from '@/pages/ReplayLab/components/ImportGuide';
...
import { CustomizeExample } from '@/pages/ReplayLab/components/Customize';
