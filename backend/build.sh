...
BuildRelease() {
	...
	for arch in ${archs[@]}
	do
		env GOOS=linux GOARCH=${arch} CGO_ENABLED=1 go build -ldflags "-linkmode external -extldflags -static" -o ./build/${project_name}-linux-${arch}
	...
	do
		env GOOS=windows GOARCH=${arch} CGO_ENABLED=1 go build -ldflags "-linkmode external -extldflags -static" -o ./build/${project_name}-win32-${arch}.exe
	...
	do
		env GOOS=darwin GOARCH=${arch} CGO_ENABLED=1 go build -ldflags "-linkmode external -extldflags -static" -o ./build/${project_name}-darwin-${arch}
